	...
	)
{
	...
	#pragma omp parallel
	{
		...
		{
			// Feed each part of the string to the stream reader:
			base64_stream_encode_url(&state, src + i * len, len, out + i * len * 4 / 3, &s);
			sum += s;
		}
	}
	...
	// Encode the remaining bytes:
	base64_stream_encode_url(&state, src + num_threads * len, last_len, out + num_threads * len * 4 / 3, &s);

	// Finalize the stream by writing trailer if any:
	base64_stream_encode_final_url(&state, out + num_threads * len * 4 / 3 + s, &t);
	...
	)
{
	...
	#pragma omp parallel
	{
		...
		for (i = 0; i < num_threads; i++)
		{
			...
			// Feed each part of the string to the stream reader:
			this_result = base64_stream_decode_url(&state, src + i * len, len, out + i * len * 3 / 4, &s);
			sum += s;
			result += this_result;
		}
	}
	...
	// initial state, since we are at a 4-byte boundary:
	state = initial_state;
	result = base64_stream_decode_url(&state, src + num_threads * len, last_len, out + num_threads * len * 3 / 4, &s);
	sum += s;
	*outlen = sum;

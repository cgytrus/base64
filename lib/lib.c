...
{
	codec.enc_url(state, src, srclen, out, outlen);
}
...
void
base64_stream_encode_final_url
	( struct base64_state	*state
	, char			*out
	, size_t		*outlen
	)
{
	...
	if (state->bytes == 1) {
		*o++ = base64_table_enc_6bit_url[state->carry];
		*o++ = '=';
		*o++ = '=';
		*outlen = 3;
		return;
...
{
	return codec.dec_url(state, src, srclen, out, outlen);
}
	...
	)
{
	...
	#ifdef _OPENMP
	if (srclen >= OMP_THRESHOLD) {
		base64_encode_openmp_url(src, srclen, out, outlen, flags);
		return;
	}
	#endif
	...
	// Feed the whole string to the stream reader:
	base64_stream_encode_url(&state, src, srclen, out, &s);

	// Finalize the stream by writing trailer if any:
	base64_stream_encode_final_url(&state, out + s, &t);
	...
	)
{
	...
	#ifdef _OPENMP
	if (srclen >= OMP_THRESHOLD) {
		return base64_decode_openmp_url(src, srclen, out, outlen, flags);
	}
	#endif
	...
	// Feed the whole string to the stream reader:
	ret = base64_stream_decode_url(&state, src, srclen, out, outlen);

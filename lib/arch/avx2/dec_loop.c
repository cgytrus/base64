...
static inline int
dec_loop_avx2_inner_url (const uint8_t **s, uint8_t **o, size_t *rounds)
{
	const __m256i lut_lo = _mm256_setr_epi8(
		0x15, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11,
		0x11, 0x11, 0x13, 0x1A, 0x1B, 0x1B, 0x1B, 0x1A,
...
static inline void
dec_loop_avx2_url (const uint8_t **s, size_t *slen, uint8_t **o, size_t *olen)
{
	if (*slen < 45) {
		return;
	}
	...
	*olen += rounds * 24;	// 24 bytes produced per round
	...
	do {
		if (rounds >= 8) {
			if (dec_loop_avx2_inner_url(s, o, &rounds) &&
			    dec_loop_avx2_inner_url(s, o, &rounds) &&
			    dec_loop_avx2_inner_url(s, o, &rounds) &&
			    dec_loop_avx2_inner_url(s, o, &rounds) &&
			    dec_loop_avx2_inner_url(s, o, &rounds) &&
			    dec_loop_avx2_inner_url(s, o, &rounds) &&

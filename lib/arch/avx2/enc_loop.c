...
static inline void
enc_loop_avx2_inner_first_url (const uint8_t **s, uint8_t **o)
{
	// First load is done at s - 0 to not get a segfault:
	__m256i src = _mm256_loadu_si256((__m256i *) *s);
...
static inline void
enc_loop_avx2_inner_url (const uint8_t **s, uint8_t **o)
{
	// Load input:
	__m256i src = _mm256_loadu_si256((__m256i *) *s);
...
static inline void
enc_loop_avx2_url (const uint8_t **s, size_t *slen, uint8_t **o, size_t *olen)
{
	if (*slen < 32) {
		return;
	}
	...
	*olen += rounds * 32;   // 32 bytes produced per round
	...
	// The first loop iteration requires special handling to ensure that
	// the read, which is done at an offset, does not underflow the buffer:
	enc_loop_avx2_inner_first_url(s, o);
	rounds--;

	while (rounds > 0) {
		if (rounds >= 8) {
			enc_loop_avx2_inner_url(s, o);
			enc_loop_avx2_inner_url(s, o);
			enc_loop_avx2_inner_url(s, o);
			enc_loop_avx2_inner_url(s, o);
			enc_loop_avx2_inner_url(s, o);

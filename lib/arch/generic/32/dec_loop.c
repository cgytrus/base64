...
static inline int
dec_loop_generic_32_inner_url (const uint8_t **s, uint8_t **o, size_t *rounds)
{
	const uint32_t str
		= base64_table_dec_32bit_d0_url[(*s)[0]]
		| base64_table_dec_32bit_d1_url[(*s)[1]]
		| base64_table_dec_32bit_d2_url[(*s)[2]]
		| base64_table_dec_32bit_d3_url[(*s)[3]];
...
static inline void
dec_loop_generic_32_url (const uint8_t **s, size_t *slen, uint8_t **o, size_t *olen)
{
	if (*slen < 8) {
		return;
	}
	...
	*olen += rounds * 3;	// 3 bytes produced per round
	...
	do {
		if (rounds >= 8) {
			if (dec_loop_generic_32_inner_url(s, o, &rounds) &&
			    dec_loop_generic_32_inner_url(s, o, &rounds) &&
			    dec_loop_generic_32_inner_url(s, o, &rounds) &&
			    dec_loop_generic_32_inner_url(s, o, &rounds) &&
			    dec_loop_generic_32_inner_url(s, o, &rounds) &&
			    dec_loop_generic_32_inner_url(s, o, &rounds) &&

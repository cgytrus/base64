...
static inline void
enc_loop_generic_32_inner_url (const uint8_t **s, uint8_t **o)
{
	uint32_t src;
...
static inline void
enc_loop_generic_32_url (const uint8_t **s, size_t *slen, uint8_t **o, size_t *olen)
{
	if (*slen < 4) {
		return;
	}
	...
	*olen += rounds * 4;	// 4 bytes produced per round
	...
	do {
		if (rounds >= 8) {
			enc_loop_generic_32_inner_url(s, o);
			enc_loop_generic_32_inner_url(s, o);
			enc_loop_generic_32_inner_url(s, o);
			enc_loop_generic_32_inner_url(s, o);
			enc_loop_generic_32_inner_url(s, o);

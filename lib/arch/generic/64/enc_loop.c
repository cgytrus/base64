...
static inline void
enc_loop_generic_64_inner_url (const uint8_t **s, uint8_t **o)
{
	uint64_t src;
	...
	// Table lookup and store:
	memcpy(*o + 0, base64_table_enc_12bit_url + index0, 2);
	memcpy(*o + 2, base64_table_enc_12bit_url + index1, 2);
	memcpy(*o + 4, base64_table_enc_12bit_url + index2, 2);
	memcpy(*o + 6, base64_table_enc_12bit_url + index3, 2);
...
static inline void
enc_loop_generic_64_url (const uint8_t **s, size_t *slen, uint8_t **o, size_t *olen)
{
	if (*slen < 8) {
		return;
	}
	...
	*olen += rounds * 8;	// 8 bytes produced per round
	...
	do {
		if (rounds >= 8) {
			enc_loop_generic_64_inner_url(s, o);
			enc_loop_generic_64_inner_url(s, o);
			enc_loop_generic_64_inner_url(s, o);
			enc_loop_generic_64_inner_url(s, o);
			enc_loop_generic_64_inner_url(s, o);
